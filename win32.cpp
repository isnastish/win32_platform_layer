//TODO:
//
//[ ] Refactor the codebase, make everything explicit!
...
function void win32_init_gdi(){
    HMODULE gdi_library = LoadLibraryA("gdi32.dll");
    if(gdi_library){
        GetStockObject = (GetStockObjectPtr)GetProcAddress(gdi_library, "GetStockObject");
        if(!GetStockObject){
            GetStockObject = get_stock_object_stub;
        }
    }
}
